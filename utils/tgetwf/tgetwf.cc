...
//                      }
		...
		if (sc(argv[index], "-averages") || sc(argv[index], "-a")
			...
			if (no_averages == 0 || no_averages == 1)
		...
		if (sc(argv[index], "-sample") || sc(argv[index], "-s")
		    || sc(argv[index], "-sam")) {
			no_averages = 0;	/* tek_scope_set_averages() interprets this as "sample mode" */
			got_no_averages = TRUE;
		}

		if (sc(argv[index], "-hires") || sc(argv[index], "-h")
		    || sc(argv[index], "-hi_res")) {
			no_averages = 1;	/* tek_scope_set_averages() interprets this as "hires mode" */
			got_no_averages = TRUE;
		}

		if (sc(argv[index], "-peak_detect") || sc(argv[index], "-p")
		    || sc(argv[index], "-peak")) {
			no_averages = -1;	/* tek_scope_set_averages() interprets this as "peak detect mode" */
			got_no_averages = TRUE;
		}

		if (sc(argv[index], "-envelope") || sc(argv[index], "-e")
		    || sc(argv[index], "-env")) {
			no_averages = -2;	/* tek_scope_set_averages() interprets this as "envelope mode" */
			got_no_averages = TRUE;
		}

		if (sc(argv[index], "-no_envelopes") || sc(argv[index], "-no_e")
		    || sc(argv[index], "-no_envs")) {
			sscanf(argv[++index], "%d", &no_averages);
		...
		if (sc(argv[index], "-repeat") || sc(argv[index], "-r")
		    || sc(argv[index], "-rep")) {
		...
		if (sc(argv[index], "-no_clear_sweeps")
		    || sc(argv[index], "-noclsw")
		    || sc(argv[index], "-no_clear")) {
			clear_sweeps = FALSE;
		}
	...
	if (got_file == FALSE || got_ip == FALSE || got_scope_channel == FALSE) {
		printf
		    ("%s: grabs a waveform from a Tektronix scope via ethernet, by Steve (Aug 09)\n",
		     progname);
		printf("Run using %s [arguments]\n\n", progname);
		printf("REQUIRED ARGUMENTS:\n");
		printf
		    ("-ip     -ip_address     -IP      : IP address of scope (eg 128.243.74.98)\n");
		printf
		    ("-f      -filename       -file    : filename (without extension)\n");
		printf
		    ("-c      -scope_channel  -channel : scope channel (1,2,3,4,M,REF1-REF4,D0-D15)\n");
		printf("OPTIONAL ARGUMENTS:\n");
		printf
		    ("-t      -timeout                 : timout (in milliseconds)\n");
		printf
		    ("-n      -no_points       -points : set maximum no of points\n");
		printf
		    ("-a      -averages        -aver   : set no of averages (<=1 means sample mode)\n");
		printf
		    ("-sa     -seg_averages    -seg_aver:set no of averages (segmented mode)\n");
		printf
		    ("-seg    -segmented       -fast   : set no of segments in segmented (FastFrame) mode\n");
		printf
		    ("-p      -peak_detect     -peak   : set to peak detect mode\n");
		printf
		    ("-s      -sample          -sam    : set to sample mode (no averaging)\n");
		printf
		    ("-h      -hires           -hi_res : set to hires mode\n");
		printf
		    ("-e      -envelope        -env    : set to envelope mode\n");
		printf
		    ("-no_e   -no_envelopes    -no_envs: sets no of envelopes (3000 scopes only)\n");
		printf
		    ("-r      -repeat          -rep    : take 'r' traces (0 means \"until 'q'\")\n");
		printf
		    ("-clsw   -clear_sweeps    -clear  : clear sweeps/'single acquisition' mode\n");
		printf
		    ("-noclsw -no_clear_sweeps -noclear: no clear sweeps (if averaging)\n\n");
		printf("OUTPUTS:\n");
	...
	if (f_wf > 0) {
		...
		if (npoints > 0) {
			actual_npoints =
			    tek_scope_set_record_length(clink, npoints);
			printf
			    ("You asked for %ld points. Record length has been set to %ld points.\n",
			     npoints, actual_npoints);
		}

		/* Set up the scope. This function also returns the no of bytes needed */
		buf_size =
		    tek_scope_set_for_capture(clink, clear_sweeps, timeout);
		if (got_segmented == TRUE) {
			...
			if (clear_sweeps == TRUE) {	//added this to try and fix noclsw issue
				actual_no_segments =
				    tek_scope_set_segmented(clink, no_segments);
				printf
				    ("Actual number used will be %d segments.\n",
				     actual_no_segments);
		...
		if (got_no_averages == TRUE) {
			...
			if (actual_no_averages > 1) {
				printf
				    ("You asked for %d averages. Actual number used will be %d averages.\n",
			...
			if (actual_no_averages == 0) {
				printf
				    ("Sample mode (no averaging) explicitly set\n");
			}
			if (actual_no_averages == -1) {
			...
			if (actual_no_averages < -1) {
				printf
				    ("Envelope mode explicitly set. On 4000-series scopes, infinite\n");
				printf
				    ("envelopes will be used. On 3000-series scopes, %d envelopes\n",
				     -actual_no_averages);
				printf("will be used.\n");
			}
		}
		...
		if (got_segmented_averages == TRUE) {
			actual_no_averages =
			    tek_scope_set_segmented_averages(clink,
							     no_averages);
			printf
			    ("You asked for %d segmented averages. Actual number used will be %d averages.\n",
			     no_averages, actual_no_averages);
		}
		...
		do {
			...
			bytes_returned =
			    tek_scope_get_data(clink, channel, clear_sweeps,
			...
			if (count != repeat) {
				printf
				    ("Trace %d acquired. Press 'Enter' to take another, or\n",
				     count);
		...
		if (got_segmented == FALSE) {
			...
			if (no_traces_acquired > 1)
				printf("A total of %d traces were acquired.\n",
				       no_traces_acquired);
		}
		fclose(f_wf);
		delete[]buf;
		...
		tek_scope_write_wfi_file(clink, wfiname, progname,
